...
class Tabular {
    ...
    getRow(idx) {
        if (idx < 0 || idx > this.size().col) { throw "Index out of bound" }
        return new Row(this, idx)
    }
    getColumn(idx) {
        if (idx < 0 || idx > this.size().row) { throw "Index out of bound" }
    ...
    error(rowIdx, colIdx, message) {
        console.log(chalk.red(`[EROR] Cell (${rowIdx},${colIdx}) : ${message}`))
    }
